...
namespace WindowsLayoutSnapshot
{
    public partial class TrayIconForm : Form
    {
        ...
        internal static ContextMenuStrip TrayMenu { get; set; }
        ...
        public TrayIconForm()
        {
            ...
            m_snapshotTimer.Tick += OnSnapshotTimerTick;
        ...
        private void UpdateRestoreChoicesInMenu()
        {
            ...
            foreach (var snapshot in snapshotsOldestFirst)
            {
                var menuItem = new RightImageToolStripMenuItem(snapshot.TimeTaken.ToLocalTime().ToString("MMM dd, h:mm tt")) { Tag = snapshot };
                ...
                if (showMonitorIcons)
                {
                    var sizes = snapshot.MonitorPixelCounts.Select(monitorPixels => (float) Math.Sqrt((float) monitorPixels / maxNumMonitorPixels));
                    monitorSizes.AddRange(sizes);
                }
            ...
            try
            {
                var textPaddingField = typeof(ToolStripDropDownMenu).GetField("TextPadding", BindingFlags.NonPublic | BindingFlags.Static); // TODO: check for null
                ...
                textPaddingField.SetValue(trayMenu,
                    new Padding(m_originalTrayMenuTextPadding.Value.Left,
                        m_originalTrayMenuTextPadding.Value.Top,
                        m_originalTrayMenuTextPadding.Value.Right - (showMonitorIcons ? 34 : 0),
                        m_originalTrayMenuTextPadding.Value.Bottom));
            }
            catch
            {
                // todo: it's not right to swallow exceptions
            ...
            try
            {
                var arrowPaddingField = typeof(ToolStripDropDownMenu).GetField("ArrowPadding", BindingFlags.NonPublic | BindingFlags.Static); // TODO: check for null
                ...
                arrowPaddingField.SetValue(trayMenu,
                    new Padding(m_originalTrayMenuArrowPadding.Value.Left,
                        m_originalTrayMenuArrowPadding.Value.Top,
        ...
        private static IEnumerable<Snapshot> CondenseSnapshots(IEnumerable<Snapshot> snapshots, int maxNumSnapshots)
        {
            if (maxNumSnapshots < 2)
            {
                throw new Exception(); // todo: why?
            }
            ...
            var condenseSnapshots = new List<Snapshot>();
            condenseSnapshots.AddRange(snapshots);
            while (condenseSnapshots.Count > maxNumSnapshots)
            {
                for (var i = 0; i < condenseSnapshots.Count; i++)
                {
                    if (condenseSnapshots[i].Age > TimeSpan.FromDays(condenseSnapshots[i].UserInitiated ? 5 : 3))
                    {
                        condenseSnapshots.RemoveAt(i);
                    }
                }
                break;
            }
            ...
            while (condenseSnapshots.Count > maxNumSnapshots)
            {
                ...
                for (var i = 1; i < condenseSnapshots.Count - 1; i++)
                {
                    var distanceBetweenNeighbors = (condenseSnapshots[i + 1].TimeTaken - condenseSnapshots[i - 1].TimeTaken).Duration();

                    if (condenseSnapshots[i].UserInitiated)
                    {
                        // a hack to make manual snapshots prioritized over automated snapshots
                        distanceBetweenNeighbors += TimeSpan.FromDays(1000000);
                    }
                    if (DateTime.UtcNow.Subtract(condenseSnapshots[i].TimeTaken).Duration() <= TimeSpan.FromHours(2))
                    {
                ...
                condenseSnapshots.RemoveAt(ixMostAdjacentNeighbors);
            }

            return condenseSnapshots;
        }
        ...
        private void OnQuitToolStripMenuItemClick(object sender, EventArgs e) => Application.Exit();
        ...
        private void OnSnapshotTimerTick(object sender, EventArgs e) => TakeSnapshot(false);

        private void OnSnapshotToolStripMenuItemClick(object sender, EventArgs e) => TakeSnapshot(true);
        ...
        private void OnJustNowToolStripMenuItemClick(object sender, EventArgs e) => m_menuShownSnapshot.Restore(null, EventArgs.Empty);

        private void OnJustNowToolStripMenuItemMouseEnter(object sender, EventArgs e) => SnapshotMousedOver(sender, e);
        ...
        private class RightImageToolStripMenuItem : ToolStripMenuItem
        {
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                foreach (var monitorSize in MonitorSizes)
                {
                    var thisIconSize = new Size((int) Math.Ceiling(maxIconSize.Width * monitorSize), (int) Math.Ceiling(maxIconSize.Height * monitorSize));
                    var thisIconLocation = new Point(nextRight - thisIconSize.Width, maxIconY + (maxIconSize.Height - thisIconSize.Height));
                    ...
                    using (var ia = new ImageAttributes())
                    {
                        ...
                        e.Graphics.DrawImage(icon,
                            new Rectangle(thisIconLocation, thisIconSize),
                            0,
                            0,
                            icon.Width,
                            icon.Height,
